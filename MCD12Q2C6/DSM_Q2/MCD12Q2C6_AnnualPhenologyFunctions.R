...
Get3YearDataChunk <- function(evi2_files, resid_files, snow_files, year_of_interest, start_line, lines_to_read){
    ...
    ## TODO - for SANJEEB to swap in this function
  #  year_function <- function(x) substr(unlist(strsplit(basename(in_name),split="\\."))[2],2,5)
...
#---------------------------------------------------------------------
WritePhenologyData <- function(out_file, pheno_values_bip, tile, nbands=2){
	...
	writeBin(as.integer(pheno_values_bip), ff, endian="little",size=2)
