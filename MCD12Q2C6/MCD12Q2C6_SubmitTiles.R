...
# parameters
years_to_do <- c(2003)
# tmp_df <- expand.grid(years_to_do, europe_tiles, stringsAsFactors=F)
tmp_df <- expand.grid(years_to_do, all_tiles, stringsAsFactors=F)

# function to submit a single tile-year job
SubFunction <- function(x){
  year <- x[1]
  tile <- x[2]
  sub_cmd <- paste("qsub -V -pe omp 16 -l h_rt=04:00:00 -l mem_total=98G /projectnb/modislc/users/joshgray/MCD12Q2C6/subAnnualPheno.sh", tile, year)
  system(sub_cmd)
}

# submit all the jobs
apply(tmp_df, 1, SubFunction)
