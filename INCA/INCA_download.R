...
GetCredentials <- function(pw_file){
  con <- file(pw_file, "r")
  username <- readLines(con, n = 1)
  password <- readLines(con, n = 1)
  close(con)
  return(list(user=username, pw=password))
}

#-------------------------------------------------------------------------------
DownloadMCD12Q2 <- function(x, out_root_dir, overwrite=FALSE, pw_file="~/earthdata_pw.txt"){
  ...
  out_dir <- file.path(out_root_dir, "MCD12Q2", as.character(x[1]), "new")
  existing_mcd12q2_files <- dir(out_dir, pattern=as.character(x[2]), full=T)
  if(length(existing_mcd12q2_files) == 0 | overwrite){
    # LPDAAC version
    # creds <- GetCredentials(pw_file)
    # # file.path("https://e4ftl01.cr.usgs.gov/MOTA/MCD12Q2.006/", paste(x[1], ".01.01", sep=""))
    # # out_dir <- "/rsstu/users/j/jmgray2/SEAL/INCA/MCD12Q2C6/"
    # wget_cmd <- paste('wget --user=', creds$user, '--password="', creds$pw, '" -e robots=off -m -np -nd -R -r -l1 -A "*', as.character(x[2]), '*hdf" "https://e4ftl01.cr.usgs.gov/MOTA/MCD12Q2.006/', x[1], '.01.01', '/" -P ', out_dir, sep="")
    # cat(wget_cmd)
...
download_df <- data.frame(expand.grid(years, all_tiles))
